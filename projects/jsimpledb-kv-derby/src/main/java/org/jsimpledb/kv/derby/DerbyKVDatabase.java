 ...
 */
package org.jsimpledb.kv.derby;
 ...
 * @author rgonzalez
 */
public class DerbyKVDatabase implements KVDatabase {
    ...
    /**
     * Option key for {@link #createTransaction(Map)}. Value should be an
     * {@link IsolationLevel} instance, or the
     * {@link IsolationLevel#name name()} thereof.
     */
    ...
    /**
     * The name of the key/value table. Default value is
     * {@value #DEFAULT_TABLE_NAME}.
     */
    protected String tableName = DEFAULT_TABLE_NAME;

    /**
     * The name of the key column. Default value is
     * {@value #DEFAULT_KEY_COLUMN_NAME}.
     */
    protected String keyColumnName = DEFAULT_KEY_COLUMN_NAME;

    /**
     * The name of the value column. Default value is
     * {@value #DEFAULT_VALUE_COLUMN_NAME}.
     */
    protected String valueColumnName = DEFAULT_VALUE_COLUMN_NAME;

    /**
     * The default transaction isolation level. Default is
     ...
     *
     * <p>
     * This may be overridden on a per-transaction basis; see
     * {@link #getIsolationLevel(Map)}.
     *
     * <p>
     ...
     * mutations during read-only transactions will be captured in memory and
     * not written to the database, and the underlying SQL transaction committed
     * on commit. Some implementations require the latter in order to guarantee
     * up-to-date reads within the transaction.
     *
     * <p>
     ...
     *
     * @param rollbackForReadOnly true to use rollback when committing a
     ...
     *
     * @param connection open database connection (will be closed by the caller
     * of this method)
...
//        final int indexSize = this.innodbLargePrefix ? INNODB_LARGE_INDEX_SIZE : INNODB_NORMAL_INDEX_SIZE;
        final int indexSize = INNODB_NORMAL_INDEX_SIZE;
//        final String rowFormat = this.innodbLargePrefix ? " ROW_FORMAT=DYNAMIC" : "";
        final String sql = "CREATE TABLE " + this.quote(this.getTableName()) + " (\n"
                + "  " + this.quote(this.getKeyColumnName()) + " VARCHAR(" + indexSize + ") FOR BIT DATA NOT NULL,\n"
                + "  " + this.quote(this.getValueColumnName()) + " BLOB NOT NULL,\n"
     ...
     * {@link #createTransactionConnection createTransactionConnection()} to get
     * a {@link Connection} for the new transaction, then invokes these methods
     * in order:
     * <ol>
     ...
     * @throws IllegalStateException if no {@link DataSource} is
     * {@linkplain #setDataSource configured}
     */
    @Override
    public DerbyKVTransaction createTransaction(Map<String, ?> options) {
     ...
     * <p>
     * The implementation in {@link SQLKVDatabase} invokes
     * {@link #createTransaction(Map) createTransaction(null)} (i.e., with no
     ...
     */
    @Override
    public DerbyKVTransaction createTransaction() {
     ...
     *
     * <p>
     * The implementation in {@link SQLKVDatabase} supports an
     * {@link IsolationLevel} under the key {@link #OPTION_ISOLATION}; also, the
     * {@link IsolationLevel} may be configured by the Spring
     * {@link org.jsimpledb.spring.JSimpleDBTransactionManager}, for example,
     * using the
     ...
     * <p>
     * The implementation in {@link SQLKVDatabase} invokes
     * {@link DataSource#getConnection()} on the
     * {@linkplain #dataSource configured} {@link DataSource}, and then
     * deletates to {@link #configureConnection configureConnection()} to
     * perform any required configuration of the new {@link Connection}.
     *
     ...
     *
     * <p>
     * The implementation in {@link SQLKVDatabase} does nothing. Note:
     * subclasses must ensure the transaction is configured for the
     * {@link IsolationLevel} configured on this instance.
     *
     ...
     * <p>
     * The implementation in {@link SQLKVDatabase} invokes
     * {@link Connection#setAutoCommit Connection.setAutoCommit(false)}.
     *
     * @param connection the {@link Connection} for a new transaction
     * @throws SQLException if an error occurs
     * @see #createSQLKVTransaction createSQLKVTransaction()
    ...
    /**
     * Create a new {@link SQLKVTransaction} for a new transaction given the
     * specified {@link Connection}. There will already be an SQL transaction
     * open on {@code connection}.
     *
     * <p>
     ...
     * @throws SQLException if an error occurs
     */
    protected DerbyKVTransaction createSQLKVTransaction(Connection connection) throws SQLException {
        return new DerbyKVTransaction(this, connection);
    }
    ...
    /**
     * Create an SQL statement that reads the key and value columns (in that
     * order) associated with the smallest key greater than or equal to
     * <code>&#63;1</code>, if any.
     *
     * @param reverse true to return rows in descending key order, false to
     * return rows in ascending key order
     * @return SQL query statement
     */
    ...
    /**
     * Create an SQL statement that reads the key and value columns (in that
     * order) associated with the greatest key strictly less than
     * <code>&#63;1</code>, if any.
     *
     * @param reverse true to return rows in descending key order, false to
     * return rows in ascending key order
    ...
    /**
     * Create an SQL statement that reads the key and value columns (in that
     * order) associated with all keys in the range <code>&#63;1</code>
     * (inclusive) to <code>&#63;2</code> (exclusive), possibly reversed.
     *
     * @param reverse true to return rows in descending key order, false to
     * return rows in ascending key order
     * @return SQL query statement
    ...
    /**
     * Create an SQL statement that reads all of the key and value columns (in
     * that order), possibly reversed.
     *
     * @param reverse true to return rows in descending key order, false to
     * return rows in ascending key order
     * @return SQL query statement
     ...
     * <code>&#63;1</code> and value <code>&#63;2</code> A row with key
     * <code>&#63;1</code> may already exist; if so, the value should be updated
     * to <code>&#63;2</code> (if syntax requires it, the value may be updated
     * to <code>&#63;3</code> instead; <code>&#63;3</code>, if it exists, will
     * be set to the same value as <code>&#63;2</code>).
     *
...
//                + this.quote(this.valueColumnName) + " = ?";
        ...
        String ck = this.quote(this.keyColumnName);
        String cv = this.quote(this.valueColumnName);
        String sql = "merge into " + tn + " dest using SYSIBM.SYSDUMMY1 on " + ck + " = ? when matched then update set " + cv + " = ? when not matched then insert values (?, ? )";
        return sql;
    }

    /**
     * Create an SQL statement that deletes the row associated with key
     * <code>&#63;1</code>, if any. Note that the key may or may not exist prior
     ...
     * <code>&#63;1</code> (inclusive} to <code>&#63;2</code> (exclusive).
     *
     * @return SQL delete statement
     */
    ...
    /**
     * Create an SQL statement that deletes all rows with keys greater than or
    ...
    /**
     * Create an SQL statement that deletes all rows with keys strictly less
     * than <code>&#63;1</code>.
     *
     * @return SQL delete statement
     */
     ...
     *
     * <p>
     * This is an optional method; returning {@code statement} unmodified is
     * acceptable, but subclasses may be able to improve efficiency by modifying
     * the SQL statement in a vendor-specific manner to only return one row.
     *
     * <p>
     * The implementation in {@link SQLKVDatabase} returns its parameter
     * unchanged.
     *
    ...
    public String limitSingleRow(String sql) {
        return sql + " FETCH FIRST ROW ONLY";
    }
    ...
    /**
     * Wrap the given {@link SQLException} in the appropriate
     * {@link KVTransactionException}.
     *
     * @param transaction the {@link SQLKVTransaction} in which the exception
     * occured
     * @param e SQL exception
     * @return appropriate {@link KVTransactionException} with chained exception
     * {@code e}
     * @throws NullPointerException if {@code e} is null
     */
    public KVTransactionException wrapException(DerbyKVTransaction transaction, SQLException e) {
